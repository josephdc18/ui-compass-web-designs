#!/usr/bin/env node
/**
 * Screenshot content-kit/facebook-cover.html at 1640×624 via Browserless.
 * Inlines src/assets/logo-on-dark.svg so the path resolves remotely.
 *
 * Usage:
 *   BROWSERLESS_API_KEY=... node scripts/screenshot-facebook-cover.mjs
 * (also reads .dev.vars)
 */
...
const HTML_PATH = resolve(ROOT, "content-kit/facebook-cover.html");
const OUT_PATH = resolve(ROOT, "content-kit/facebook-cover.png");
...
function inlineLogo(html) {
  ...
  return html.replace(
    /src=("[^"]*logo-on-dark\.svg"|'[^']*logo-on-dark\.svg')/g,
    `src="${LOGO_DATA_URI}"`
  );
}
...
const res = await fetch(`${BASE}/screenshot?token=${encodeURIComponent(TOKEN)}`, {
  ...
  body: JSON.stringify({
    ...
    viewport: { width: 1640, height: 624, deviceScaleFactor: 2 },
