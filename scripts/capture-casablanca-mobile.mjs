#!/usr/bin/env node
/** Screenshot the live Casablanca site at a phone viewport via Browserless. */
...
const OUT_PATH = resolve(ROOT, "content-kit/casablanca-mobile.png");
...
if (!TOKEN) { console.error("Missing BROWSERLESS_API_KEY"); process.exit(1); }

const res = await fetch(`${BASE}/screenshot?token=${encodeURIComponent(TOKEN)}`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    url: "https://casablancabarbershoptx.com",
...
if (!res.ok) { console.error(`Browserless ${res.status}: ${(await res.text()).slice(0, 400)}`); process.exit(1); }
